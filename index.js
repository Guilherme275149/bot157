...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus Man,Não Foi Nada Bom Ter Você Aqui No Grupo@${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛*_Rlx Ae Mano Em Quando Carrega Não Gostaria De Toma Uma Xicare De Chá?Se Não For Espere 5 Min E Tente Novamente⌛',
				success: '✔️Sucesso✔️',
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, Adicionando Como Admin Da Bagaca Aqui Caraio:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Aqui Está A Lista De Números Bloqueado:\n'
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 BOTZINMAKER 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'tchau Amg Vai Com Deus👋', text) // ur cods
					}, 0)
                     break
